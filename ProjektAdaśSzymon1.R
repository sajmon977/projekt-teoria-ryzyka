...
#zadanie 4
USbond_ <- read.csv2(file="USbond.csv") #wczytanie danych rentownosci obligacji
USbond_[,1] <- as.Date(USbond_[,1],"%d.%m.%Y") #wczytanie 1-szej kolumny jako data
USbond <- USbond_[nrow(USbond_):1,] #notowana jest rentownosc obligacji. Byly w odwrotnej kolejnosci wiec je zamieniamy
databond <- sapply(dane_all[dates,1], function(x) sum(USbond[,1] < x) + 1) #wektor gdzie kolejne wiersze sa poczatkiem kolejnej inwestycji, czyli poczatek kazdego miesiaca
...
sp <- read.csv2(file="S&P500.csv") #wczytanie danych indeksu S&P500
sp[,1] <- as.Date(sp[,1],"%d.%m.%Y") #wczytanie 1-szej kolumny jako data 
sp[,2] <- as.numeric(as.character(sp[,2])) #wczytanie 2-giej kolumny jako liczba
spdsz <- sp[-1,2]/sp[-dim(sp)[1],2] -1 #policzenie dziennych stop zwrotu z S&P500
...
for(i in 1:24){
  portfelSP500[i+1] <- portfelSP500[i]*(1+stopyzwrotu[i,3])
  portfelbonds[i+1] <- portfelbonds[i]*(1+stopyzwrotu[i,4])
...
legend("topleft", legend=c("porfelX", "porfelY","portfelSP500","portfelbonds"), col=c(1,2,3,4), lty=1, cex=0.7) #dodanie legendy

odchs <-sd(gielda_all[13:36]) #policzylismy odchylenia z miesiecznych rynkowych stop zwrotu dla okresu, w ktorych inwestujemy.
sharpe_all <- matrix(lapply(1:3, function(i) (mean(stopyzwrotu[,i])-mean(stopyzwrotu[,4]))/odchs), ncol=3,dimnames = list(NULL,c("portfelX","portfelY","S&P500")))
#policzylismy sharpe_ratio dla portfeli X,Y i S&P500
Sharpe_brzeg <- matrix(lapply(1:8, function(i) (mean(portfeleBrzegowe_ret_all[,i])-mean(stopyzwrotu[,4]))/odchs), ncol=8,dimnames = list(NULL,colnames(portfeleBrzegowe_ret_all)))
#Sharpe_ratio dla portfeli brzegowych

xtable(as.data.frame(c(sharpe_all,Sharpe_brzeg),col.names = c(colnames(sharpe_all),colnames(Sharpe_brzeg)))) #tabelka shape ratio do latexa 


#CAPM, zle, liczone stopy zwrotu z portfela na podstawie praktycznych stop zwrotu z rynku, szczegoly w pdf-ie.
alfa_all1 <- matrix(ncol=2,dimnames = list(NULL,c("alfa_allX","alfa_allY")))[-1,] #wartosci alfa i beta CAPM-u dla portfelaX
beta_all1 <- matrix(ncol=2,dimnames = list(NULL,c("alfa_allX","alfa_allY")))[-1,] #wartosci alfa i beta CAPM-u dla portfelaY
...
for(i in 1:24){
  ...
  alfa_all1 = rbind(alfa_all1,c(CAPM1$coefficients[1],CAPM21$coefficients[1]))
  beta_all1 = rbind(beta_all1,c(CAPM1$coefficients[2],CAPM21$coefficients[2]))
...
plot(dane_all[dates[13:36],1],result_CAPM1[,1],type = "l",ylim=c(-0.15,0.15),xlab = "czas", ylab = "stopa zwrotu") #wykres wymodelowanych stop zwrotu przez CAPM
lines(dane_all[dates[13:36],1],stopyzwrotu[,1], col="blue") #dodanie prawdziwych stop zwrotu
plot(dane_all[dates[13:36],1],result_CAPM1[,2],type = "l",ylim=c(-0.15,0.15),xlab = "czas", ylab = "stopa zwrotu") #wykres wymodelowanych stop zwrotu przez CAPM
lines(dane_all[dates[13:36],1],stopyzwrotu[,2], col="blue") #dodanie prawdziwych stop zwrotu


#CAPM2 wlasciwie policzony
alfa_all <- matrix(ncol=2,dimnames = list(NULL,c("alfa_allX","alfa_allY")))[-1,] #wartosci alfa i beta CAPM-u dla portfelaX
beta_all <- matrix(ncol=2,dimnames = list(NULL,c("beta_allX","beta_allY")))[-1,] #wartosci alfa i beta CAPM-u dla portfelaY
...
for(i in 1:24){
  ...
  alfa_all = rbind(alfa_all,c(CAPM$coefficients[1],CAPM2$coefficients[1]))
  beta_all = rbind(beta_all,c(CAPM$coefficients[2],CAPM2$coefficients[2]))
} #dla modelu CAPM liczymy bety i alfy
...
plot(dane_all[dates[13:36],1],result_CAPM[,1],ylim=c(-0.15,0.15),type = "l",xlab = "czas", ylab = "stopa zwrotu") #wykres wymodelowanych stop zwrotu przez CAPM
lines(dane_all[dates[13:36],1],stopyzwrotu[,1], col="blue") #dodanie prawdziwych stop zwrotu
legend(dane_all[dates[22],1],-0.08,legend=c("CAPM", "faktyczna stopa ","zwrotu z portfelaX"), col=c(1,"blue","white"),pt.lwd = 10, y.intersp = 0.2, x.intersp = 0.3, lty=1, cex=2, bty = 'n') #dodanie legendy

plot(dane_all[dates[13:36],1],result_CAPM[,2],ylim=c(-0.15,0.15),type = "l",xlab = "czas", ylab = "stopa zwrotu") #wykres wymodelowanych stop zwrotu przez CAPM
lines(dane_all[dates[13:36],1],stopyzwrotu[,2], col="blue") #dodanie prawdziwych stop zwrotu
legend(dane_all[dates[22],1],-0.08,legend=c("CAPM", "faktyczna stopa ","zwrotu z portfelaY"), col=c(1,"blue","white"),pt.lwd = 10,y.intersp = 0.2, x.intersp = 0.3, lty=1, cex=2, bty = 'n') #dodanie legendy
...
} #dodanie punktow portfeli brzegowych
